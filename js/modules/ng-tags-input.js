...
tagsInput.directive('autoComplete', ["$document","$timeout","$sce","tagsInputConfig", function($document, $timeout, $sce, tagsInputConfig) {
    ...
    return {
        ...
        link: function(scope, element, attrs, tagsInputCtrl) {
            ...
            var newSuggestionItemsTemplate = "<div> <div class=\"pull-left\"><p>%text%</p><p>%ldesc%</p></div> <div class=\"pull-right\"> <p>%rdesc%</p> </div> </div>";
            ...
            scope.highlight = function(item,index) {
                ...
                var completeItem = suggestionList.completeItems[index]; 
                var newHtml = scope.replaceNewHtmlValues(completeItem,text,index);
                return $sce.trustAsHtml(newHtml);
            };
            
            scope.replaceNewHtmlValues = function(completeItem,text,index){
            	var result  = newSuggestionItemsTemplate.replace("%text%",text);
            	var result  = result.replace("%ldesc%",completeItem.ldesc);
            	var result  = result.replace("%rdesc%",completeItem.rdesc);
            	return result;
            };
...
tagsInput.run(["$templateCache", function($templateCache) {
    ...
    "<div class=\"host\" tabindex=\"-1\" ti-transclude-append=\"\"><div class=\"tags\" ng-class=\"{focused: hasFocus}\"><ul class=\"tag-list\"><li class=\"tag-item\" ng-style=\"tag.getChildColor\" ng-repeat=\"tag in tagList.items track by track(tag)\" ng-class=\"{ selected: tag == tagList.selected }\"><span ng-bind=\"getDisplayText(tag)\"></span> <a class=\"remove-button\" ng-click=\"tagList.remove($index)\" ng-bind=\"options.removeTagSymbol\"></a></li></ul><input class=\"input\" ng-model=\"newTag.text\" ng-change=\"newTagChange()\" ng-trim=\"false\" ng-class=\"{'invalid-tag': newTag.invalid}\" ti-bind-attrs=\"{type: options.type, placeholder: options.placeholder, tabindex: options.tabindex}\" ti-autosize=\"\"></div></div>"
  );

  $templateCache.put('ngTagsInput/auto-complete.html',
    "<div class=\"autocomplete\" ng-show=\"suggestionList.visible\"><ul class=\"suggestion-list\"><li class=\"suggestion-item\" ng-class-even=\"'suggestionListOddClass'\" ng-repeat=\"item in suggestionList.items track by track(item)\" ng-class=\"{selected: item == suggestionList.selected}\" ng-click=\"addSuggestionByIndex($index)\" ng-mouseenter=\"suggestionList.select($index)\" ng-bind-html=\"highlight(item,$index)\"></li></ul></div>"
  );
}]);
